...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 12 14:16:46 2021
...
def Get_weights(directory):
    ...
    # loss_weights = class_weight.compute_class_weight('balanced', uni_label,
                                                    # train_flat)
    class_sample_count = np.unique(train_flat, return_counts=True)[1]

    freq_c = class_sample_count/np.sum(class_sample_count)

    a_c = np.median(freq_c)/freq_c
    # class_sample_count = np.unique(train_flat, return_counts=True)[1]
    # weight = 1. / class_sample_count
    # samples_weight = weight[train_labels]
    # samples_weight = torch.from_numpy(samples_weight)
    return a_c # loss_weights
